...
def api_design(state: TheoryCraftState) -> dict:
    ...
    result, error = safe_structured_call(APIRoutesOutput, messages, temperature=0.1)
    if result is None:
        return {"api_routes_draft": [], "errors": [f"api_design failed: {error}"]}
    return {"api_routes_draft": [r.model_dump(by_alias=False) for r in result.routes]}
