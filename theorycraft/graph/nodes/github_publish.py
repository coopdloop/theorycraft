...
def github_publish(state: TheoryCraftState) -> dict:
    ...
    try:
        ...
        if publish_mode == "gist":
            url = client.create_gist(session_name, content)
        elif publish_mode == "pr":
            existing_repo = state.get("github_existing_repo", "")
            url = client.create_pr(existing_repo, session_name, content)
        else:
            url = client.create_repo(session_name, content, org=cfg.github_org)
